...
Multiplex in Alfafar (Valencia). Uses the reservaentradas.com platform.
The cinema listing page at reservaentradas.com/cine/alfafar/cinesmn4 is
server-rendered and provides all /sesiones/ links — no browser needed.

Strategy:
1. GET reservaentradas.com/cine/alfafar/cinesmn4  → collect /sesiones/ links
2. For each film, GET its session page  → tabs = days, session-container = times
3. Skip links marked ?proximamente=true (upcoming, no sessions yet).
...
CINEMA_KEY  = "mn4"
CINEMA_NAME = "Cines MN4"
BASE        = "https://www.reservaentradas.com"
LIST_URL    = f"{BASE}/cine/alfafar/cinesmn4"
...
def _parse_tab_date(text: str) -> str:
    """'Do 10 / 05' → 'YYYY-MM-DD'; bumps year if month already passed."""
...
def _scrape_film(session: requests.Session, ses_url: str) -> dict | None:
    ...
    # Poster: lazy-loaded img hosted on cinesmn4.reservaentradas.com
    poster = ""
    for img in soup.select("img[data-original], img[src]"):
        src = img.get("data-original") or img.get("src", "")
        if "cinesmn4.reservaentradas.com" in src or "cinesmn4" in src:
            poster = src
            break
...
def scrape_mn4() -> list[dict]:
    """Scrape Cines MN4 and return a list of film dicts."""
    log.info("Fetching Cines MN4 film list …")
    ...
    log.info("Cines MN4: %d films, %d sessions", len(results), total)
    return results
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"Cines MN4  --  {len(films)} films  /  {total} sessions")
