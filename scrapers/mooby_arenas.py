"""
Mooby Arenas de Barcelona scraper.
...
CINEMA_KEY  = "mooby_arenas"
CINEMA_NAME = "Mooby Arenas"
...
def scrape_mooby_arenas(playwright=None) -> list[dict]:
    log.info("Fetching Mooby Arenas (moobycinemas.com) …")
    try:
        html = _fetch_html()
    ...
    if not venue:
        log.error("Could not find Mooby Arenas venue in window.shops (slug %s)", VENUE_SLUG)
        return []

    theater_id = str(venue.get("id", ""))
    shop_url_tpl = venue.get("shop_url", "")
    events = venue.get("events", [])
    log.info("Mooby Arenas: %d events found", len(events))
    ...
    log.info("Mooby Arenas: %d events, %d sessions", len(results), total)
    return results


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    films = scrape_mooby_arenas()
    total = sum(len(f["showtimes"]) for f in films)
    vose  = [f for f in films if f["is_vose"]]
    print(f"\n{'='*60}")
    print(f"Mooby Arenas  --  {len(films)} films  /  {total} sessions")
